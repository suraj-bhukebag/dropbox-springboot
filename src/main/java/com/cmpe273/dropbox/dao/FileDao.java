...
public interface FileDao extends CrudRepository<com.cmpe273.dropbox.model.Files, Long> {
	...
	@Query(value = "SELECT * FROM files WHERE link = ?1", nativeQuery = true)
	public Files findByLink(@Param("link") String link);
	
	@Modifying
	@Query(value = "DELETE FROM files WHERE created_by_id = ?1 and path = ?2 and name = ?3", nativeQuery = true)
	public void deleteByCreatedByAndPathAndName(@Param("created_by_id") long created_by_id, @Param("path") String path, @Param("name") String name);
